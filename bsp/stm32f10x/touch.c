...
static rt_uint16_t LCD_X_SIZE = 0, LCD_Y_SIZE = 0;
...
//SPI写数据
static void WriteDataTo7846(unsigned char num)
{
    SPI_WriteByte(num);
}
...
static void rtgui_touch_calculate()
{
    if (touch != RT_NULL)
    {
        ...
        if (touch->calibrating != RT_TRUE)
        {
            ...
            {
                touch->x = (touch->x - touch->min_x) * LCD_X_SIZE/(touch->max_x - touch->min_x);
            }
            else if (touch->max_x < touch->min_x)
            {
                touch->x = (touch->min_x - touch->x) * LCD_X_SIZE/(touch->min_x - touch->max_x);
            }

            if (touch->max_y > touch->min_y)
            {
                touch->y = (touch->y - touch->min_y) * LCD_Y_SIZE /(touch->max_y - touch->min_y);
            }
            else if (touch->max_y < touch->min_y)
            {
                touch->y = (touch->min_y - touch->y) * LCD_Y_SIZE /(touch->min_y - touch->max_y);
            }

            // normalize the data
            if (touch->x & 0x8000)
                touch->x = 0;
            else if (touch->x > LCD_X_SIZE)
                touch->x = LCD_X_SIZE - 1;

            if (touch->y & 0x8000)
                touch->y = 0;
            else if (touch->y > LCD_Y_SIZE)
                touch->y = LCD_Y_SIZE - 1;
        }
    }
}
...
void touch_timeout(void* parameter)
{
    ...
    /* touch time is too short and we lost the position already. disable the timer and enable the interrupt */
...
void rtgui_touch_hw_init(void)
{
    /* SPI1 config */
    {
        ...
        SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_16;/* 72M/16=4.5M */
    ...
    {
        struct rtgui_rect rc;

        rtgui_graphic_driver_get_rect(rtgui_graphic_driver_get_default(), &rc);
        LCD_X_SIZE = rc.x2;
        LCD_Y_SIZE = rc.y2;
    }
}
